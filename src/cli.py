...
def __validate_path(type_, value):
    ...
    if value.suffix not in [".tif", ".tiff"] or not value.stem.endswith("_qa"):
        raise ValueError("File must be a Planet Labs ARPS QA image.")

@app.command
def info(
    image_path: Annotated[Path, Parameter(validator=__validate_path)],
):
    """Get tide height info for a Planet Labs ARPS raster image.
    ...
    print(f"Acquisition Time: {tides_info['acquisition_time'].strftime('%Y-%m-%d %H:%M:%S')}")
